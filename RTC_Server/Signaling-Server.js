...
module.exports = exports = function(app, socketCallback) {
    ...
    var chatIO=io.of('/chat');
        ...
        chatIO.on('connection',function(socket){
          ...
          socket.emit("success-connect","success connect");

          socket.on("initUserInfo",function(data){
            ...
            try{
              ...
              socket.fontColor=getRandomColor();
              socket.userImg=data.userImg;
              socket.broadcast.to(socket.roomId).emit("joinUser",{nickname:socket.nickname});
            }catch(e){
              console.log(e);
            }
        ...
        //클라이언트에서 채팅메시지 전송
          socket.on("sendChatMessage",function(data){
            socket.broadcast.to(socket.roomId).emit("receiveChatMessage",{nickname:socket.nickname,
                                                                          fontColor:socket.fontColor,
                                                                          userImg:socket.userImg,
                                                                          message:data});
          });
          //채금금지
          socket.on("chatMute",function(data){
            socket.broadcast.to(socket.roomId).emit("chatMute",data);
          });
          ...
          //인원변경
          socket.on("changCurrentUser",function(data){
            console.log(data);
            socket.broadcast.to(socket.roomId).emit("changCurrentUser",data);
          });
